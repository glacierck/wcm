...
package com.kongrongqi.shopmall.wedget.progressBar;
...
public class TextRoundCornerProgressBar extends BaseRoundCornerProgressBar implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    @Override
    protected void drawProgress(LinearLayout layoutProgress, float max, float progress, float totalWidth, float totalHight,
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            if (max >= 30 && progress <= 4 && progress>0) {
                layoutProgress.setBackgroundResource(R.drawable.schduler_min);
                ViewGroup.LayoutParams lParams = layoutProgress.getLayoutParams();
                lParams.width = LayoutParams.WRAP_CONTENT;
            } else {
                layoutProgress.setBackground(backgroundDrawable);
                measureWidth(layoutProgress, max, progress, totalWidth, totalHight, padding);
            }
        } else {
            if (max >= 30 && progress <= 4 && progress>0) {
                layoutProgress.setBackgroundResource(R.drawable.schduler_min);
    ...
    private void measureWidth(LinearLayout layoutProgress, float max, float progress, float totalWidth, float totalHight, int padding) {
        float ratio = max / progress;
        int progressWidth = (int) ((totalWidth - (padding * 2)) / ratio);
        ViewGroup.LayoutParams progressParams = layoutProgress.getLayoutParams();
        Logger.d("高度", totalHight + "");
...
//    }
    ...
    private void alignTextProgressOutsideProgress() {
        ...
        if (isReverse()) {
            ...
            params.addRule(RelativeLayout.CENTER_HORIZONTAL, R.id.layout_progress);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1)
                params.addRule(RelativeLayout.CENTER_HORIZONTAL, R.id.layout_progress);
        }
        tvProgress.setLayoutParams(params);
